...
import { cloudConfigured, requireSupabase } from './supabase';
...
function toSnakeCaseRecord(value: Record<string, unknown>) {
  return Object.fromEntries(
    Object.entries(value).map(([key, entry]) => [key.replace(/[A-Z]/g, (letter) => `_${letter.toLowerCase()}`), entry]),
  );
}

function toCamelCaseRecord<T>(value: Record<string, unknown>): T {
  return Object.fromEntries(
    Object.entries(value).map(([key, entry]) => [key.replace(/_([a-z])/g, (_, letter: string) => letter.toUpperCase()), entry]),
  ) as T;
}
...
export const cloudRepository = {
  ...
  async loadSnapshot(shopId: string): Promise<CloudSnapshot> {
    const [customers, repairs, inventory] = await Promise.all([
      fetchTable<Customer>('customers', shopId),
      fetchTable<Repair>('repairs', shopId),
      fetchTable<InventoryItem>('inventory_items', shopId),
    ]);
    return { customers, repairs, inventory };
  },

  async saveCustomers(shopId: string, customers: Customer[]) {
    await upsertTable('customers', shopId, customers as unknown as Array<Record<string, unknown>>);
  },

  async saveRepairs(shopId: string, repairs: Repair[]) {
    await upsertTable('repairs', shopId, repairs as unknown as Array<Record<string, unknown>>);
  },

  async saveInventory(shopId: string, inventory: InventoryItem[]) {
    await upsertTable('inventory_items', shopId, inventory as unknown as Array<Record<string, unknown>>);
  },

  subscribe(shopId: string, onChange: () => void) {
    const client = requireSupabase();
    const channel = client
      .channel(`gadgetpos-shop-${shopId}`)
