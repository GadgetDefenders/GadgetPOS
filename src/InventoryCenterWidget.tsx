...
import { AlertTriangle, Archive, Boxes, ClipboardList, PackageCheck, Plus, RotateCcw, Search, Trash2, Truck, X } from 'lucide-react';
import { storage } from './storage';
import type { InventoryItem } from './types';

type Lifecycle='Active'|'Archived'|'Discontinued'|'On Order';
type ItemMeta={supplier?:string;location?:string;reorderQty?:number;taxable?:boolean;repairPrice?:number;status?:Lifecycle;archivedAt?:string};
type StockEntry={id:string;itemId:string;itemName:string;type:'Received'|'Adjustment'|'Sold'|'Returned'|'Damaged'|'Archived'|'Restored'|'Discontinued'|'Deleted';quantity:number;note:string;createdAt:string};
...
export default function InventoryCenterWidget(){
 ...
 const [lifecycle,setLifecycle]=useState<'All'|Lifecycle>('Active');
 ...
 useEffect(()=>{
  ...
  const refresh=()=>setInventory(storage.getInventory());
 ...
 const statusOf=(id:string):Lifecycle=>meta[id]?.status||'Active';
 const activeInventory=inventory.filter(i=>statusOf(i.id)==='Active');
 ...
 const filtered=useMemo(()=>inventory.filter(i=>{
  ...
  const lifecycleOk=lifecycle==='All'||statusOf(i.id)===lifecycle;
  return categoryOk&&stockOk&&lifecycleOk&&hay.includes(query.toLowerCase());
 }),[inventory,meta,query,category,stock,lifecycle]);
 const costValue=activeInventory.reduce((s,i)=>s+i.quantity*Number(i.cost||0),0);
 const retailValue=activeInventory.reduce((s,i)=>s+i.quantity*Number(i.price||0),0);
 const low=activeInventory.filter(i=>i.quantity>0&&i.quantity<=i.minimum).length;
 const out=activeInventory.filter(i=>i.quantity<=0).length;
 const archived=inventory.filter(i=>statusOf(i.id)==='Archived').length;
 const ordered=orders.filter(o=>o.status==='Ordered').reduce((s,o)=>s+o.lines.reduce((a,l)=>a+l.quantity,0),0);
 const allFilteredSelected=filtered.length>0&&filtered.every(i=>selected.includes(i.id));

 function updateMeta(id:string,patch:ItemMeta){saveMeta({...meta,[id]:{...meta[id],...patch}})}
 function log(items:InventoryItem[],type:StockEntry['type'],note:string){const now=new Date().toISOString();saveHistory([...items.map(i=>({id:crypto.randomUUID(),itemId:i.id,itemName:i.name,type,quantity:0,note,createdAt:now})),...history])}
 function setLifecycleFor(ids:string[],status:Lifecycle){if(!ids.length)return;const now=new Date().toISOString();const next={...meta};ids.forEach(id=>next[id]={...next[id],status,archivedAt:status==='Archived'?now:undefined});saveMeta(next);const items=inventory.filter(i=>ids.includes(i.id));log(items,status==='Archived'?'Archived':status==='Discontinued'?'Discontinued':'Restored',status==='Active'?'Returned to active inventory':`Marked ${status.toLowerCase()}`);setSelected([])}
 function deleteSelected(){if(!selected.length)return;if(!confirm(`Permanently delete ${selected.length} selected inventory item${selected.length===1?'':'s'}? This cannot be undone.`))return;const deleting=inventory.filter(i=>selected.includes(i.id));const nextInventory=inventory.filter(i=>!selected.includes(i.id));storage.saveInventory(nextInventory);setInventory(nextInventory);const nextMeta={...meta};selected.forEach(id=>delete nextMeta[id]);saveMeta(nextMeta);log(deleting,'Deleted','Permanently deleted from inventory');setSelected([]);window.dispatchEvent(new Event('gadgetpos-data-changed'))}
 ...
 function addPOLine(){const first=activeInventory[0];if(!first)return;setPoLines([...poLines,{itemId:first.id,quantity:1,cost:Number(first.cost)||0}])}
 function savePO(){if(!poSupplier.trim()||!poLines.length){alert('Enter a supplier and add at least one item.');return}const po:PurchaseOrder={id:crypto.randomUUID(),number:`PO-${String(orders.length+1).padStart(4,'0')}`,supplier:poSupplier.trim(),expectedDate:poExpected,status:'Ordered',lines:poLines,createdAt:new Date().toISOString()};saveOrders([po,...orders]);const next={...meta};poLines.forEach(l=>next[l.itemId]={...next[l.itemId],status:'On Order'});saveMeta(next);setPoOpen(false);setPoSupplier('');setPoExpected('');setPoLines([])}
 function receivePO(po:PurchaseOrder){if(po.status==='Received')return;const qty=new Map(po.lines.map(l=>[l.itemId,l.quantity]));const next=inventory.map(i=>qty.has(i.id)?{...i,quantity:i.quantity+(qty.get(i.id)||0),cost:po.lines.find(l=>l.itemId===i.id)?.cost??i.cost,updatedAt:new Date().toISOString()}:i);storage.saveInventory(next);setInventory(next);const entries=po.lines.map(l=>{const item=inventory.find(i=>i.id===l.itemId);return{id:crypto.randomUUID(),itemId:l.itemId,itemName:item?.name||'Inventory Item',type:'Received' as const,quantity:l.quantity,note:`Received ${po.number} from ${po.supplier}`,createdAt:new Date().toISOString()}});saveHistory([...entries,...history]);saveOrders(orders.map(o=>o.id===po.id?{...o,status:'Received'}:o));const nextMeta={...meta};po.lines.forEach(l=>nextMeta[l.itemId]={...nextMeta[l.itemId],status:'Active'});saveMeta(nextMeta);window.dispatchEvent(new Event('gadgetpos-data-changed'))}
 if(!visible)return null;
 return <section className="ic-shell">
  <div className="ic-head"><div><h2>Inventory Management Center</h2><p>Stock, purchasing, lifecycle cleanup, suppliers, locations, and movement history.</p></div><div className="ic-head-actions"><button onClick={()=>window.dispatchEvent(new CustomEvent('gadgetpos-open-inventory-form'))}><Plus size={17}/>Add Item</button><button className="primary" onClick={()=>setPoOpen(true)}><Truck size={17}/>New Purchase Order</button></div></div>
  <div className="ic-metrics"><article><Boxes/><span>Active Inventory Cost</span><strong>{money(costValue)}</strong></article><article><PackageCheck/><span>Active Retail Value</span><strong>{money(retailValue)}</strong></article><article><AlertTriangle/><span>Low Stock</span><strong>{low}</strong></article><article><Archive/><span>Archived Items</span><strong>{archived}</strong></article><article><ClipboardList/><span>Units Ordered</span><strong>{ordered}</strong></article></div>
  ...
  {tab==='inventory'&&<><div className="ic-toolbar lifecycle"><label><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search item, SKU, barcode, IMEI, supplier or shelf..."/></label><select value={category} onChange={e=>setCategory(e.target.value)}>{categories.map(c=><option key={c}>{c}</option>)}</select><select value={stock} onChange={e=>setStock(e.target.value)}><option>All</option><option>In Stock</option><option>Low</option><option>Out</option></select><select value={lifecycle} onChange={e=>setLifecycle(e.target.value as 'All'|Lifecycle)}><option>All</option><option>Active</option><option>Archived</option><option>Discontinued</option><option>On Order</option></select></div>
  {selected.length>0&&<div className="ic-bulk"><strong>{selected.length} selected</strong><button onClick={()=>setLifecycleFor(selected,'Archived')}><Archive size={15}/>Archive</button><button onClick={()=>setLifecycleFor(selected,'Active')}><RotateCcw size={15}/>Restore</button><button onClick={()=>setLifecycleFor(selected,'Discontinued')}>Discontinue</button><button className="danger" onClick={deleteSelected}><Trash2 size={15}/>Delete Permanently</button></div>}
  <div className="ic-table-wrap"><table><thead><tr><th><input type="checkbox" checked={allFilteredSelected} onChange={()=>setSelected(allFilteredSelected?selected.filter(id=>!filtered.some(i=>i.id===id)):Array.from(new Set([...selected,...filtered.map(i=>i.id)])))}/></th><th>Item</th><th>Status</th><th>SKU / IMEI</th><th>Supplier</th><th>Location</th><th>Stock</th><th>Cost</th><th>Price</th><th>Potential Profit</th><th></th></tr></thead><tbody>{filtered.map(i=>{const itemStatus=statusOf(i.id);return <tr key={i.id} className={`${i.quantity<=0?'out':i.quantity<=i.minimum?'low':''} ${itemStatus.toLowerCase().replace(' ','-')}`}><td><input type="checkbox" checked={selected.includes(i.id)} onChange={()=>setSelected(selected.includes(i.id)?selected.filter(id=>id!==i.id):[...selected,i.id])}/></td><td><strong>{i.name}</strong><small>{[i.category,i.brand,i.model].filter(Boolean).join(' · ')}</small></td><td><span className={`ic-status ${itemStatus.toLowerCase().replace(' ','-')}`}>{itemStatus}</span></td><td>{i.sku||i.imei||i.barcode||'—'}</td><td><input value={meta[i.id]?.supplier||''} onChange={e=>updateMeta(i.id,{supplier:e.target.value})} placeholder="Supplier"/></td><td><input value={meta[i.id]?.location||''} onChange={e=>updateMeta(i.id,{location:e.target.value})} placeholder="Shelf / bin"/></td><td><strong>{i.quantity}</strong><small>Reorder at {i.minimum}</small></td><td>{money(i.cost)}</td><td>{money(i.price)}</td><td>{money((i.price-i.cost)*i.quantity)}</td><td><button onClick={()=>setAdjustItem(i)}>Adjust</button></td></tr>})}</tbody></table>{!filtered.length&&<div className="ic-empty">No inventory matches these filters.</div>}</div></>}
