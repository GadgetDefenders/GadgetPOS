...
import { readPosCatalog, type PosCatalogItem } from './PosCatalogManagerWidget';
...
export default function App(){
 ...
 const [customCatalog,setCustomCatalog]=useState<PosCatalogItem[]>(readPosCatalog());
 ...
 useEffect(()=>{
   ...
   const refreshCatalog=()=>setCustomCatalog(readPosCatalog());
   ...
   window.addEventListener('gadgetpos-pos-catalog-changed',refreshCatalog);
   ...
   return()=>{
     ...
     window.removeEventListener('gadgetpos-pos-catalog-changed',refreshCatalog);
 ...
 const customTiles=useMemo(()=>customCatalog.filter(i=>i.tab===posTab&&!i.hidden).sort((a,b)=>a.order-b.order),[customCatalog,posTab]);
 ...
 function quickItem(name:string,price:number,taxable=true){setCart(current=>[...current,{id:crypto.randomUUID(),kind:'Inventory',referenceId:`quick-${crypto.randomUUID()}`,description:name,quantity:1,unitPrice:price,taxable}]);}
 function addCustomItem(item:PosCatalogItem){quickItem(item.name,item.price,item.taxable)}
